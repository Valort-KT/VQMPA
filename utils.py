...
def print_peak_memory(prefix, device):
    if device == 0:
        print(f"{prefix}_{device}: {torch.cuda.max_memory_allocated(device) // 1e6}MB ")
    # elif device ==1 :
    #     print(f"{prefix}_{device}: {torch.cuda.max_memory_allocated(device) // 1e6}MB ")
...
def save_checkpoint(config, epoch, model, epoch_accuracy, max_accuracy, optimizer, logger, model_ema=None):
    if model_ema is not None:
        ...
        save_state = {'model': model.state_dict(),
                    ...
                    'input': input,
                    'config': config}
    pth_dir = os.path.join(config.OUTPUT, 'checkpoints')
    if not os.path.exists(pth_dir):
        os.makedirs(pth_dir)

    save_path = os.path.join(pth_dir, f'ckpt_epoch_{epoch}.pth')
    best_path = os.path.join(pth_dir, f'best.pth')
